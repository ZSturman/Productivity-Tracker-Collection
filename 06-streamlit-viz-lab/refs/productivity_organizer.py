...
with st.expander("Productivity CSV"):
    st.write(productivity_csv)
with st.expander("Timeline CSV"):
    st.write(timeline_csv)
with st.expander("Session_states"):
    st.write(st.session_state)
with st.expander("Relationships CSV"):
    st.write(relationships_csv)
...
def load_data():
    df = pd.read_csv('productivity/csvs/timeline.csv')
    df.loc[df['started'] == False, 'started'] = None
    df.loc[df['completed'] == False, 'completed'] = None

    df['started'] = pd.to_datetime(df['started'], errors='coerce')
    df['completed'] = pd.to_datetime(df['completed'], errors='coerce')
    df['due_date'] = pd.to_datetime(df['due_date'])
    
    df['end_date_estimated'] = df['started'] + pd.to_timedelta(df['estimated_duration'], unit='d')
    df.loc[df['completed'].notnull(), 'end_date_actual'] = df['completed']
    df.loc[df['completed'].isnull(), 'end_date_actual'] = df['started'] + pd.to_timedelta(df['actual_duration'], unit='d')
